...
export default {
    template: `
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in pb-10 items-start">
            ...
            <div class="bg-surface-dark/80 backdrop-blur-md border border-white/10 p-8 rounded-[2rem] shadow-2xl relative overflow-hidden flex flex-col group h-fit">
                ...
                <div class="relative z-10 flex flex-col h-fit">
                        ...
                        <div class="text-center">
                            ...
                            <div class="px-2 relative group/slider">
                                <div class="relative h-10 flex items-center">
                                    <!-- Track Background -->
                                    <div class="absolute inset-x-0 h-3 bg-black/40 rounded-full border border-white/10 overflow-hidden shadow-inner"></div>
                    ...
                    <div class="grid grid-cols-3 gap-3 mb-8 shrink-0">
                        <div class="bg-black/20 p-3 rounded-xl border border-white/10 flex flex-col items-center justify-center gap-1 backdrop-blur-sm group hover:bg-white/5 transition-colors">
            ...
            <div class="flex flex-col gap-6">
                 <div class="bg-surface-dark/80 backdrop-blur-md border border-white/10 rounded-[2rem] shadow-xl overflow-hidden flex flex-col h-fit relative">
                    <div class="p-6 border-b border-white/10 bg-black/20">
                        <h3 class="text-lg font-bold text-white flex items-center gap-3">
                            <i class="fas fa-file-invoice-dollar text-accent"></i>
                            Aktif Kredilerim
                            <span class="px-2 py-0.5 rounded-lg bg-white/10 text-white text-xs font-mono border border-white/10">{{ store.loans.length }}</span>
                        </h3>
                    </div>
                    
                    <!-- Empty State -->
                    <div v-if="store.loans.length === 0" class="flex flex-col items-center justify-center py-12 text-center opacity-50">
                        <div class="w-20 h-20 rounded-full bg-white/5 border border-white/15 border-dashed flex items-center justify-center mb-4">
                    ...
                    <div v-else class="overflow-y-auto px-4 py-4 space-y-3 custom-scrollbar relative z-10 max-h-[500px]">
                        <div v-for="loan in store.loans" :key="loan.id"
                             class="group bg-white/[0.02] border border-white/10 hover:border-accent/30 hover:bg-white/[0.05] rounded-xl p-4 transition-all duration-300 relative overflow-hidden">
                            ...
                            <div class="flex items-center justify-between gap-4 relative z-10">
                                ...
                                <div class="flex items-center gap-4 min-w-0 flex-1">
                                    <div class="w-12 h-12 rounded-xl bg-slate-800 flex items-center justify-center border border-white/15 text-slate-400 group-hover:text-accent group-hover:border-accent/30 transition-colors shrink-0 shadow-inner">
                                    ...
                                    <div class="min-w-0">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-[10px] px-1.5 py-0.5 rounded bg-black/30 text-slate-400 border border-white/10 font-mono hidden sm:inline-block">
                                                {{ loan.date }}
                                            </span>
                                        </div>
                                ...
                                <div class="flex flex-col items-end gap-1 shrink-0">
                                    ...
                                    <button class="flex items-center gap-2 bg-slate-800 hover:bg-accent border border-white/15 hover:border-accent/50 text-white pl-3 pr-4 py-2 rounded-lg transition-all group/btn shadow-sm hover:shadow-accent/20 active:scale-95">
